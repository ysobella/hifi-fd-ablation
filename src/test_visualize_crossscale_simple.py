"""
Hifi-FD Cross-Scale Consistency Visualization Script (Simplified)

This script visualizes broken cross-scale consistency in the Hifi-FD model by analyzing
multi-scale SRM features, cross-scale correlations, and DCMA attention maps. This is a
simplified version that focuses on reliably capturing the key features.
...
Usage:
    python test_visualize_crossscale_simple.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description='Visualize cross-scale consistency in Hifi-FD model (simplified)')
...
class SimpleCrossScaleAnalyzer:
    """
    Simplified analyzer class for extracting cross-scale consistency features.
    """
    ...
    def __init__(self, model, device):
        ...
        # Register hooks to capture SRM features
        self._register_hooks()
    
    def _register_hooks(self):
        """Register forward hooks to capture SRM features."""
...
def create_comprehensive_visualization(analysis_result, true_label, category, sample_idx, output_dir):
    ...
    Args:
        analysis_result (dict): Results from SimpleCrossScaleAnalyzer
...
def main():
    ...
    # Initialize analyzer
    analyzer = SimpleCrossScaleAnalyzer(model, device)
